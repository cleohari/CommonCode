...
class FilterTest extends PHPUnit\Framework\TestCase
{
    ...
    {
        $filter = new \Flipside\Data\Filter('a eq b');
        ...
        $filter = new \Flipside\Data\Filter('a eq 1');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '=');
        ...
        $filter = new \Flipside\Data\Filter('a ne b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '!=');
        ...
        $filter = new \Flipside\Data\Filter('a gt b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '>');
        ...
        $filter = new \Flipside\Data\Filter('a ge b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '>=');
        ...
        $filter = new \Flipside\Data\Filter('a lt b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '<');
        ...
        $filter = new \Flipside\Data\Filter('a le b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '<=');
        ...
        $filter = new \Flipside\Data\Filter('a xy b');
        $clauses = $filter->getChildren();
        $this->assertCount(1, $clauses);
        $clause = $clauses[0];
    ...
    {
        $filter = new \Flipside\Data\Filter('a eq b and c eq d');
        $clauses = $filter->getChildren();
    ...
    {
        $filter = new \Flipside\Data\Filter('a eq b or c eq d');
        $clauses = $filter->getChildren();
        $this->assertCount(3, $clauses);
        $clause = $clauses[0];
        $this->assertEquals($clause->op, '=');
    ...
    public function testSQL()
    {
        ...
        $filter = new \Flipside\Data\Filter('contains(a,b)');
        $sql = $filter->to_sql_string();
    ...
    {
        $filter = new \Flipside\Data\Filter('a eq b and c eq d or e eq f');
        try
        {
        ...
        $filter = new \Flipside\Data\Filter('a eq b or c eq d and e eq f');
        try
        {
            $filter->to_ldap_string();
            $this->assertFalse(true);
    ...
    public function testMongo()
    {
        ...
        $filter = new \Flipside\Data\Filter('_id eq 4af9f23d8ead0e1d32000000');
        ...
        {
            $comp = array('_id'=>new \MongoDB\BSON\ObjectId('4af9f23d8ead0e1d32000000'));
            $this->assertEquals($comp, $filter->to_mongo_filter());
        }

        $filter = new \Flipside\Data\Filter('substringof(a,b)');
        ...
        {
            $this->assertEquals($regex->getPattern(), 'b');
            $this->assertEquals($regex->getFlags(), 'i');
        }

        $filter = new \Flipside\Data\Filter('indexof(a,b)');
        $mongo = $filter->to_mongo_filter();
        $this->assertEquals(array('a'=>'b'), $mongo);

        $filter = new \Flipside\Data\Filter('indexof(tolower(a),b)');
        $mongo = $filter->to_mongo_filter();
        $this->assertArrayHasKey('a', $mongo);
        $array = $mongo['a'];
        $this->assertArrayHasKey('$regex', $array);
        ...
        $filter = new \Flipside\Data\Filter('a eq true');
        $mongo = $filter->to_mongo_filter();
        $this->assertArrayHasKey('a', $mongo);
        $this->assertTrue($mongo['a']);

        $filter = new \Flipside\Data\Filter('a eq false');
        $mongo = $filter->to_mongo_filter();
        $this->assertArrayHasKey('a', $mongo);
        ...
        $filter = new \Flipside\Data\Filter();
        $mongo = $filter->to_mongo_filter();
    ...
    public function testPHP()
    {
        ...
        $filter = new \Flipside\Data\Filter('a ne 1');
        $array = array(array('a'=>1),array('a'=>2));
        $test = $filter->filter_array($array);
        $this->assertCount(1, $test);
        $this->assertEquals(array('a'=>2), $test[0]);

        $filter = new \Flipside\Data\Filter('a lt 1');
        $array = array(array('a'=>1),array('a'=>2));
        $test = $filter->filter_array($array);
        $this->assertCount(0, $test);

        $filter = new \Flipside\Data\Filter('a le 1');
        $array = array(array('a'=>1),array('a'=>2));
        $test = $filter->filter_array($array);
        $this->assertCount(1, $test);
        $this->assertEquals(array('a'=>1), $test[0]);

        $filter = new \Flipside\Data\Filter('a gt 1');
        $array = array(array('a'=>1),array('a'=>2));
        $test = $filter->filter_array($array);
        $this->assertCount(1, $test);
        $this->assertEquals(array('a'=>2), $test[0]);

        $filter = new \Flipside\Data\Filter('a ge 1');
        $array = array(array('a'=>1),array('a'=>2));
        $test = $filter->filter_array($array);
    ...
    {
        $filter = new \Flipside\Data\Filter('year eq current and test eq a');
